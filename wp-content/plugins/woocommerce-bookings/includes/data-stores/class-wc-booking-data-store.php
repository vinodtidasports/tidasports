	...
	*/
	...
	public function read( &$booking ) {
		...
		if ( ! $booking_id || ! $post_object || 'wc_booking' !== $post_object->post_type ) {
			...
			throw new Exception( sprintf( esc_html__( 'The booking object could not be read. This probably means the booking no longer exists or does not match the expected type. Here is the booking object: %s', 'woocommerce-bookings' ), print_r( $booking, true ) ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r, WordPress.Security.EscapeOutput.ExceptionNotEscaped
		}
	 ...
	 *
	 * @return int[] Array of booking IDs.
	 */
	...
	public static function get_booking_ids_by( $filters = array() ) {
		...
		$filters = wp_parse_args(
			...
			array(
				...
				'date_start_before'        => false,
				'date_end_after'           => false,
				'order'                    => 'DESC',
				'date_before'              => false,
				'date_after'               => false,
		...
		if ( ! empty( $filters['date_start_before'] ) ) {
			$meta_keys[]   = '_booking_start';
			$query_where[] = "_booking_start.meta_value < '" . esc_sql( date( 'YmdHis', $filters['date_start_before'] ) ) . "'"; // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
		}

		if ( ! empty( $filters['date_end_after'] ) ) {
			$meta_keys[]   = '_booking_end';
			$query_where[] = "_booking_end.meta_value > '" . esc_sql( date( 'YmdHis', $filters['date_end_after'] ) ) . "'"; // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
		}
