...
class WC_Bookings_Single_Export {
	...
	public function catch_export_requests() {
		...
		// Check if we are exporting a valid product with a valid nonce action from bookings plugin.
		if ( ! $product_id || "export_product_with_global_rules_$product_id" !== $nonce_action ) {
			return;
		}
