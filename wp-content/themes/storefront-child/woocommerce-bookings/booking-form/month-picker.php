...
<div class="form-field form-field-wide current_resource_<?php echo esc_attr( $current_resource ); ?>">
	...
	<?php endif; ?>
	<ul class="block-picker month-picker 12345">
		<?php
		foreach ( $blocks as $block => $available ) {
			...
			echo '<li class="123456789  ' . esc_attr( $fully_booked_class ) . esc_attr( $unavailable_class ) . '" data-block="' . esc_attr( date( 'Ym', $block ) ) . '" title="' . esc_attr( $title_attribute ) . '"><a href="#" data-value="' . esc_attr( date( 'Y-m', $block ) ) . '">' . esc_html( date_i18n( 'M Y', $block ) ) . '</a></li>';
		}
		?>
	</ul>
