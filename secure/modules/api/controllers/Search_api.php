...
class Search_api extends REST_Controller  {
    ...
    {       
        ...
        if($venues){
            ...
            foreach($venues as $key=>$venue){ 
                ...
                $is_approved = $this->Users_Model->getTableData('tbl_user',$where);
                if($is_approved ){ 
                if($is_like){                          
            ...
            $academyDetails = $this->Users_Model->SearchnearbyRecord('tbl_academy',$latitude,$longitude,$distance_km); 
            $experienceDetails = $this->Users_Model->SearchnearbyRecord('tbl_experience',$latitude,$longitude,$distance_km);
            foreach($experienceDetails as $key=>$experienceDetail){ 
                $u_id = $experienceDetail->user_id;
                $where = ['id'=>$u_id,'status'=>'1'];
                $is_approved = $this->Users_Model->getTableData('tbl_user',$where);
                if($is_approved ){ 
                if($is_like){                          
            ...
            $tournamentDetails = $this->Users_Model->SearchnearbyRecord('tbl_tournament',$latitude,$longitude,$distance_km);
            if($tournamentDetails){
            foreach($tournamentDetails as $key=>$tournamentDetail){ 
                if($tournamentDetail->image){
                    $tournamentDetail->image =   'https://tidasports.com/secure/uploads/tbl_tournament/'.$tournamentDetail->image ;         
                }
                $u_id = $tournamentDetail->user_id;
                $where = ['id'=>$u_id,'status'=>'1'];
                $is_approved = $this->Users_Model->getTableData('tbl_user',$where);
                if($is_approved ){
                if($tournamentDetail->approved == 1){
                if($is_like){                          
                    $keys = array('post_id','type','user_id','is_like');
    ...
    {       
        ...
        if($tournaments){
            foreach($tournaments as $key=>$tournament){  
                if($tournament->image){
                    $tournament->image =   'https://tidasports.com/secure/uploads/tbl_tournament/'.$tournament->image ;         
                }
                if($is_like){                          
                    $keys = array('post_id','type','user_id','is_like');
    ...
    {       
        ...
        $getprofiledetail = array();
        $search = $fields = '';
        $academies = $this->Users_Model->getactiveTblData('tbl_academy','sports',$sport); 
        ...
        if($academies){
            ...
            foreach($academies as $key=>$academy){  
                ...
                if($academy->logo){
                    $academy->logo =   'https://tidasports.com/secure/uploads/tbl_academy/'.$academy->logo ;            
                }
