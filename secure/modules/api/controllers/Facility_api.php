...
class Facility_api extends REST_Controller  {
	...
	public function insertData_post()
	{
		...
		if($available_24_hours == "0")
		{
		$arrayclosingtime = explode(":",$closing_time);
		$arrayopeningtime = explode(":",$opening_time);
		$diffhr = intval($arrayclosingtime[0]) - intval($arrayopeningtime[0]);
		$diffmin = intval($arrayclosingtime[1]) - intval($arrayopeningtime[1]);
		if($diffhr < 0)
		{
			$diffhr += 24;
		}
		...
		//echo $interval;
		$numberofslots = intval( $interval / intval($slot_length_min));
		//echo $numberofslots;
		for($i=1; $i <= $numberofslots; $i++)
		{
			$closetime = explode(':', $clostm);
// 			echo $clostm;
			$hours = intval($closetime[0]); 
			$minutes = intval($closetime[1]) + intval($slot_length_min);
			if ($minutes >= 60) {
              $hours += intval($minutes/60);
              $minutes = $minutes % 60;
             }
			 $hours = $hours % 24;
			 $clstm = implode(":",[$hours,$minutes]);
			 //echo $clstm; 
		$data = array(
			'title' => $title,
			'venue_id' => $venue_id,
  ...
  public function deleteData_post()
	...
	public function getFacilitySlots_post()
	{
        ...
        $facility_name = trim($this->input->post('facility_name'));
        $venue_id = trim($this->input->post('venue_id'));
		$date = trim($this->input->post('date'));
		if($facility_name != '' && $date != '' && $venue_id != '')
		{
			$where = ['status'=>1,'title'=>$facility_name, 'venue_id' => $venue_id];
			$getfacilitydetail = $this->Users_Model->getTableData('tbl_facilities',$where);
            //print_r($getfacilitydetail);
            $getfacilitydeta = $this->Users_Model->getTableData('tbl_facilities',$where);
			if($getfacilitydetail){                
				...
				$finalslots = array();
                foreach($getfacilitydetail as $singlesl)
                {
                    $bookg_where = ['status' => 1, 'facility_id' => $singlesl->id, 'booking_status' => 1, 'date'=> $date];
                    $booked_sl = $this-> Users_Model->getTableData('tbl_facility_booking',$bookg_where);
					$bkl_sl = [];
					
					if(!empty($booked_sl))
					{
						$time_loop = [];
						$time_loop['facility_id'] = $singlesl->id;
						$time_loop['slot_start_time'] = $singlesl->opening_time;
						$time_loop['slot_end_time'] = $singlesl->closing_time;
						$time_loop['slot_block_status'] = 1;
						$time_loop['slot_booking_status'] = 1;
						$bkl_sl['slot_booking_detail'] = $booked_sl[0];
						$usnwhere = $booked_sl[0] -> user_id; 
					    $user = $this ->Users_Model->getTableData('tbl_user',['id'=>$usnwhere]);
						$time_loop['slot_booking_detail'] = $booked_sl[0];
						$time_loop['user'] = $user[0];
						array_push($finalslots,$time_loop); 
					}
					else if(empty($booked_sl))
					{
						$time_loop = [];
						$time_loop['facility_id'] = $singlesl->id;
						$time_loop['slot_start_time'] = $singlesl->opening_time;
						$time_loop['slot_end_time'] = $singlesl->closing_time;
						$time_loop['slot_block_status'] = 0;
						$time_loop['slot_booking_status'] = 0;
						$time_loop['slot_booking_detail'] = null;
						$time_loop['user'] = null;
						array_push($finalslots,$time_loop); 
					}
					// echo $bookg_where['facility_id'];
				// echo $getfacilitydetail[0];
                }
	...
	public function payment_post()
	{
		...
		$payment_status = $this->input->post('payment_status');// 1 Paid, 2 Pending Payment
		if(!$payment_status){
			$payment_status = 2;
		}
		$orderdata = array(
		...
		if($insertbooking)
		{
			...
			{
				$status = ($payment_status == '1' || $payment_status == 'paid')?1:3;
				...
				//update order
				$order_update_data = array('payment_status'=>1,'transaction_id'=>$inserttransaction,'status'=>$status);
