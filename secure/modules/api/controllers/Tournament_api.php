...
class Tournament_api extends REST_Controller  {
    ...
    public function getAllData_post()
    {
        ...
        if($getprofiledetail){
                foreach($getprofiledetail as $key=>$single){ 
                    ...
                    $key = 'post_id';
                    $post_id = $single->id;
                    $where = ['post_id'=>$post_id,'post_type'=>'2'];
    ...
    public function getApprovedData_post()
    {
        ...
        if($latitude && $longitude && $distance){
            $where = ' status = 1 AND approved = ' . $approved ;
            $getprofiledetail = $this->Users_Model->getTableData($tbl,$where,$latitude,$longitude,$distance); 
        }else{
            $getprofiledetail = $this->Users_Model->getTableData($tbl,$where);
        }
    ...
    public function getacademyData_post()
    {
        ...
        if($latitude && $longitude && $distance){
            $where = ' status = 1 AND academy_id = ' . $academy_id ;
            if($id){
                $where = '  id = ' . $id ;
            }
            $getprofiledetail = $this->Users_Model->getTableData($tbl,$where,$latitude,$longitude,$distance); 
